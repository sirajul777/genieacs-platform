...
var (
	...
	ErrNoJob           = repository.ErrNoPendingJob
)

type Service struct { jobs repository.JobRepository; events repository.JobEventRepository }
func NewService(jobs repository.JobRepository, events repository.JobEventRepository) *Service { return &Service{jobs: jobs, events: events} }
type CreateInput struct { Type string; Payload []byte }
type AssignNextInput struct{ AgentID string }
type ProgressInput struct { ID string; Progress int }
type CompleteInput struct { ID string; Result []byte }
type FailInput struct { ID string; Error string }

func (s *Service) Create(ctx context.Context, input CreateInput) (domain.Job, error) {
	if input.Type == "" { return domain.Job{}, ErrInvalidJob }
...
func (s *Service) Progress(ctx context.Context, input ProgressInput) (domain.Job, error) {
	if input.Progress < 0 || input.Progress > 100 { return domain.Job{}, ErrInvalidProgress }
...
func newID() string {
	buf:=make([]byte,16); _,_=rand.Read(buf); buf[6]=(buf[6]&0x0f)|0x40; buf[8]=(buf[8]&0x3f)|0x80
