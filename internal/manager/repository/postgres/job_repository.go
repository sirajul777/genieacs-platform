...
func NewJobRepository(pool *pgxpool.Pool) *JobRepository {
	return &JobRepository{pool: pool, queries: sqlc.New(pool)}
}
...
func jobToDomain(item sqlc.Job) domain.Job {
	return domain.Job{ID: item.ID, Type: item.Type, Payload: item.Payload, Status: domain.Status(item.Status), AgentID: item.AgentID, Progress: item.Progress, Result: item.Result, Error: item.Error, CreatedAt: item.CreatedAt, UpdatedAt: item.UpdatedAt}
}

type JobEventRepository struct{ queries *sqlc.Queries }

func NewJobEventRepository(pool *pgxpool.Pool) *JobEventRepository {
	return &JobEventRepository{queries: sqlc.New(pool)}
}
